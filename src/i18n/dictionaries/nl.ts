export const nl = {
  header: {
    language: 'Taal',
    currency: 'Valuta',
    apply: 'Toepassen',
    onThisPage: 'Op deze pagina',
  },
  footer: {
    explore: 'Ontdekken',
    getStarted: 'Aan de slag',
    content: 'Inhoud',
    journal: 'Journal',
  },
  evidence: {
    claim: 'Bewering',
    strength: 'Bewijskracht',
    note: 'Opmerkingen',
    levels: {
      '1': 'Zeer beperkt',
      '2': 'Beperkt',
      '3': 'Wisselend',
      '4': 'Goed',
      '5': 'Sterk',
    },
  },
  research: {
    source: 'Bronstudie',
    doi: 'Bekijk de studie',
    sections: {
      background: 'Achtergrond',
      studyDesign: 'Opzet van de studie',
      keyFindings: 'Belangrijkste bevindingen',
      mechanism: 'Mechanisme',
      clinicalImplications: 'Klinische betekenis',
      limitations: 'Beperkingen en open vragen',
      inPlainLanguage: 'In gewone taal',
    },
  },
  lexicon: {
    sections: {
      inSimpleTerms: 'Eenvoudig gezegd',
      scientificBackground: 'Wetenschappelijke achtergrond',
      roleInGutHealth: 'Rol in de darmgezondheid',
    },
    alsoKnownAs: 'Ook bekend als',
    reviewedFor: 'Wetenschappelijk beoordeeld door',
    relatedTerms: 'Gerelateerde begrippen',
    readMore: 'Lees meer',
    filterLabel: 'Begrippen filteren',
    filterPlaceholder: '{terms} filteren, bijv. butyraat',
    searchLabel: 'Zoek in het lexicon',
    searchPlaceholder: 'Zoek in {terms}, bijv. butyraat',
    countOne: '{count} begrip',
    countAll: '{count} begrippen',
    countFiltered: '{shown} van {total} begrippen',
    searching: 'Zoeken…',
    resultsCapped: 'De eerste {shown} van {total} resultaten. Blijf typen om te verfijnen.',
    noMatch: 'Geen begrippen komen overeen met dit filter.',
    jumpToLetter: 'Naar letter springen',
    backToTop: 'Naar boven',
    browseByCategory: 'Bekijk per categorie',
    allTerms: 'Alle begrippen A–Z',
  },
  theCase: {
    readMore: 'Lees meer',
    back: 'Terug',
  },
  gutFirst: {
    revealsLabel: 'Wat jouw darmen onthullen',
    yourGut: 'Jouw darmen',
  },
  twoModels: {
    theyLabel: 'Alle anderen',
  },
  keyTakeaways: {
    heading: 'Belangrijkste inzichten',
  },
  disclaimer: {
    text: 'De inzichten van de NB1 Microbioomtest zijn uitsluitend informatief en vervangen geen professioneel medisch advies. Raadpleeg altijd een gekwalificeerde zorgverlener voordat je je voeding of supplementenroutine wijzigt.',
  },
  faq: {
    heading: 'Veelgestelde vragen',
  },
  dataTable: {
    glossary: {
      termHeader: 'Term',
      definitionHeader: 'Betekenis',
    },
  },
  cta: {
    heading: 'Optimaliseer je biologie',
    buttonText: 'Bestel je NB1 Microbioom Discovery Kit',
  },
  forms: {
    thankYouRegistering: 'Bedankt voor je registratie!',
    contactMessagePlaceholder: 'Hoe kunnen we je helpen?',
    contactSubmit: 'Bericht versturen',
    contactSending: 'Versturen…',
  },
  scienceBoard: {
    viewBio: 'Biografie bekijken',
  },
  plans: {
    months: { 1: '1 maand', 4: '4 maanden', 12: '12 maanden' } as Record<1 | 4 | 12, string>,
    orWord: 'of', // conjunction in the "4 or 12 months" tab subtitle

    perMonth: '/mnd',
    savingsPrefix: 'Bespaar',
    savingsSuffix: '/ cyclus',
    bestValue: 'Beste keuze',
    compareShow: 'Core & Advanced uitgebreid vergelijken',
    compareHide: 'Vergelijking verbergen',
    guaranteeBilledMonthly: 'Maandelijkse rekening.',
    guaranteeNoUpfront: 'Geen vooruitbetaling.',
    guaranteePayNothing: 'Je betaalt niets tot je formule is geproduceerd.',
  },
  countries: {
    Germany: 'Duitsland',
    Austria: 'Oostenrijk',
    Netherlands: 'Nederland',
    Belgium: 'België',
    France: 'Frankrijk',
    Luxembourg: 'Luxemburg',
    Ireland: 'Ierland',
    'United Kingdom': 'Verenigd Koninkrijk',
    'United Arab Emirates': 'Verenigde Arabische Emiraten',
    Switzerland: 'Zwitserland',
    Romania: 'Roemenië',
  } as Record<string, string>,
  promo: {
    codes: {
      WELCOME10: '10% korting op je eerste cyclus',
      NB1: '15% korting op je eerste cyclus',
      HYROX: '20% korting op je eerste cyclus',
      FREEMONTH: 'Eerste maand gratis',
      FIRSTMONTHFREE: 'Eerste maand gratis',
    } as Record<string, string>,
    appliedTemplate: '✓ {code} toegepast.',
    invalid: 'Deze code is niet geldig.',
    messages: {
      notFound: 'Kortingscode niet gevonden.',
      valid: 'Kortingscode is geldig.',
      referralFirstMonthFree: '✓ Eerste maand gratis met je referral-code.',
    } as Record<string, string>,
  },
  checkout: {
    hero: {
      titlePrefix: 'Bijna ',
      titleAccent: 'klaar.',
      subtitle:
        'Vul hier je gegevens in en kies de verzend- en betaalmethode. Vandaag betaal je nog niets.',
    },
    steps: {
      email: 'Jouw e-mail',
      address: 'Waar mogen we de kit naartoe sturen?',
      shipping: 'Verzendopties',
      payment: 'Betaling',
    },
    next: 'Volgende →',
    edit: 'Bewerken',
    required: 'Verplicht.',
    nameInvalid: 'Voer een geldige naam in.',
    email: {
      label: 'E-mail',
      placeholder: 'jij@email.nl',
      hint: 'We maken je account aan en sturen hier je trackinginformatie en resultaten naartoe. Zodra je de kit hebt ontvangen, stel je een wachtwoord in.',
      invalid: 'Voer een geldig e-mailadres in.',
      typoSuggestion: 'Bedoelde je {domain}?',
      useSuggestion: 'Gebruik dit',
    },
    address: {
      firstName: 'Voornaam',
      lastName: 'Achternaam',
      country: 'Land',
      addressLabel: 'Adres',
      addressPlaceholder: 'Straat en huisnummer',
      apt: 'Appartement, etage enz.',
      optional: '(optioneel)',
      postalCode: 'Postcode',
      city: 'Plaats',
      phone: 'Telefoon',
      phoneNote: '(voor bezorgupdates)',
      phonePlaceholder: '+31 …',
      phoneInvalid: 'Voer een geldig telefoonnummer in voor het geselecteerde land.',
    },
    shipping: {
      standardName: 'Standaard',
      standardSub: '3–5 werkdagen · met tracking',
      standardPrice: 'Gratis',
      expressName: 'Express',
      expressSub: '1–2 werkdagen · met tracking',
      expressPrice: '€9',
    },
    payment: {
      applePay: 'Apple Pay',
      googlePay: 'Pay',
      orPayAnotherWay: 'of betaal op een andere manier',
      card: 'Kaart',
      cardNumber: 'Kaartnummer',
      cardNumberInvalid: 'Voer een geldig kaartnummer in.',
      expiry: 'Vervaldatum',
      expiryFormat: 'MM / JJ.',
      cvc: 'CVC',
      cvcDigits: '3–4 cijfers.',
      nameOnCard: 'Naam op de kaart',
      paypalNote:
        'Je wordt naar PayPal doorgestuurd om je abonnement goed te keuren. Vandaag betaal je nog niets, de eerste kosten worden pas in rekening gebracht als je formule in productie gaat.',
      klarna: 'Klarna',
      klarnaNote:
        'Betaal met Klarna: achteraf op factuur of direct via bankoverschrijving. Je keurt de betaling goed in Klarna; het bedrag wordt pas afgeschreven bij productie.',
      sepa: 'SEPA-incasso',
      iban: 'IBAN',
      ibanPlaceholder: 'NL00 BANK 0000 0000 00',
      ibanInvalid: 'Voer een geldige IBAN in.',
      accountHolder: 'Rekeninghouder',
      sepaConsent:
        'Door je IBAN op te geven, machtig je NB1 en Stripe om je bank opdracht te geven het bedrag via SEPA-incasso van je rekening af te schrijven, pas vanaf het moment dat je formule wordt geproduceerd.',
      billingSame: 'Factuuradres gelijk aan bezorgadres',
      streetNumber: 'Straat & nummer',
      billingIndividual: 'Particulier',
      billingCompany: 'Bedrijf',
      companyName: 'Bedrijfsnaam',
      taxId: 'BTW / Fiscaal nummer',
      registrationNumber: 'Registratienummer',
    },
    secured: 'Beveiligd door Stripe',
    promoUi: {
      switchTemplate: 'Overstappen naar {plan} {duration} · {price}/mnd',
      excludeOneMonth: 'Deze code is alleen geldig voor looptijden van 4 of 12 maanden.',
      addCode: 'Kortingscode toevoegen',
      changeCode: 'Code wijzigen',
      removeCode: 'Verwijderen',
      placeholder: 'Kortingscode',
      apply: 'Toepassen',
      appliedSuffix: '{code} toegepast',
      creatorApplied: 'Creator-aanbieding toegepast',
      offerUnavailable:
        'We konden deze aanbieding niet in je browser opslaan. Schakel siteopslag in en probeer het opnieuw.',
      discount: 'Korting',
      firstMonth: 'Gratis maand',
    },
    confirm: {
      label: 'Bevestigen — {zeroPrice} vandaag te betalen',
      paypal: 'Doorgaan met PayPal →',
      klarna: 'Doorgaan met Klarna →',
      processing: 'Verwerken…',
      legalPrefix: 'Door te bevestigen ga je akkoord met onze',
      terms: 'Algemene voorwaarden',
      and: 'en ons',
      privacyPolicy: 'Privacybeleid',
      legalMid:
        ', en stem je ermee in dat NB1 je biologische monster en gezondheidsgegevens verwerkt om je formule samen te stellen. Je eerste betaling vindt pas plaats op het moment dat je formule in productie gaat, ongeveer twee weken nadat je je monster hebt teruggestuurd. Je betaalt eenmalig een bedrag van ',
      feeBold: '€49',
      legalEnd:
        ' wanneer je het monster niet binnen 4 weken terugstuurt. NB1 is voor volwassenen vanaf 18 jaar.',
      accountExists: 'Er bestaat al een account met dit e-mailadres. Meld je aan.',
      accountError: 'Je account kon niet worden aangemaakt. Probeer het opnieuw.',
      checkDetails: 'Controleer je gegevens en probeer het opnieuw.',
      orderError: 'Je bestelling kon niet worden afgerond:',
    },
    whatsNext: {
      heading: 'De volgende stappen',
      step1:
        'Jouw kit wordt verzonden (een darmtest van twee minuten) en stuur je, afgesloten in het zakje, eenvoudig terug in het originele doosje.',
      step2: 'Wij analyseren de sample en ons science team keurt je formule goed.',
      step3: 'Eerste betaling, daarna worden jouw unieke supplementen verzonden.',
    },
    summary: {
      title: 'Jouw bestelling',
      plan: 'Abonnement',
      duration: 'Looptijd',
      billing: 'Facturering',
      billingMonthly: 'Maandelijks gefactureerd',
      cancelAnytime: 'Altijd opzegbaar',
      shipping: 'Verzending',
      monthly: 'Maandelijks',
      firstCycle: 'Eerste cyclus',
      editLink: 'Abonnement of looptijd wijzigen',
      dueToday: '{zeroPrice} vandaag te betalen',
      note: 'De eerste afschrijving vindt pas plaats (ongeveer twee weken nadat je jouw sample hebt teruggestuurd) wanneer je formule daadwerkelijk in productie gaat. Nooit eerder. Een eenmalige vergoeding van € 49,- wordt alleen in rekening gebracht als de sample niet binnen 4 weken wordt teruggestuurd.',
    },
    done: {
      heading: 'Je bent erbij.',
      body: 'Je kit is onderweg. Je betaalt nu nog niets, je eerste betaling vindt pas plaats zodra je formule in productie gaat.',
      eyebrow: 'Bestelling',
      inboxTitle: 'Bekijk je inbox',
      inboxBody:
        'We hebben je bevestiging naar {email} gestuurd. Open deze om je wachtwoord in te stellen en je dashboard te openen, zo hoef je nu nog geen wachtwoord te onthouden.',
      dashboard: 'Naar mijn dashboard →',
      trackOrder: 'Bestelling volgen',
      helpLink: 'Hulp nodig?',
      failed: {
        heading: 'Betaling mislukt',
        body: 'Er is iets misgegaan met je betaling. Er is niets in rekening gebracht. Probeer het opnieuw of neem contact op.',
        retry: 'Opnieuw proberen →',
        help: 'Hulp nodig?',
      },
      supportLine: 'of',
      supportEmail: 'support@nb1.com',
      chatUs: '💬 Chat met ons',
      survey: {
        eyebrow: 'Help ons groeien',
        question: 'Hoe heb je NB1 gevonden?',
        sub: 'Met slechts één vraag help je ons om meer mensen zoals jij te bereiken.',
        thanks: 'Genoteerd, bedankt.',
        thanksSub: 'Dat helpt ons om meer mensen zoals jij te bereiken.',
        whichOne: 'Welke?',
        whichSub: 'Optioneel — tik voor een precies antwoord.',
        somethingElse: 'Iets anders',
        placeholder: 'Vertel ons waar…',
        send: 'Verzenden',
        skip: 'Overslaan',
      },
      timeline: {
        heading: 'De volgende stappen',
        step1: {
          label: 'Vandaag',
          title: 'Bestelling bevestigd',
          body: 'Bekijk je inbox, we hebben je een aankoopbewijs en een link gestuurd om je wachtwoord in te stellen en de verzendstatus van je kit te volgen.',
        },
        step2: {
          label: '~3 dagen',
          title: 'Jouw kit komt aan',
          body: 'Een darmtest van twee minuten, om afgesloten in het zakje terug te sturen in het originele doosje, plus een korte online gezondheidsvragenlijst.',
        },
        step3: {
          label: 'Week 1–2',
          title: 'Wij analyseren je sample',
          body: 'Geanalyseerd op soortniveau, waarna ons science team jouw gepersonaliseerde formule samenstelt op basis van jouw data.',
        },
        step4: {
          label: 'Week 3',
          title: 'Formule goedgekeurd · eerste betaling',
          badge: 'Eerste betaling',
          body: 'Je formule is goedgekeurd en pas op dit moment (wanneer deze in productie gaat) wordt de eerste betaling afgeschreven. Nooit eerder.',
        },
        step5: {
          label: 'Week 4',
          title: 'Jouw formule wordt verzonden',
          body: 'Activate, Restore en Nourish, in blister voor 30 dagen, klaar voor onderweg. Jouw eerste cyclus begint.',
        },
      },
      summary: {
        heading: 'Besteloverzicht',
        plan: 'Abonnement',
        cycle: 'Cyclus',
        delivery: 'Bezorging',
        deliveryValue: 'Met tracking · gratis',
        monthly: 'Maandelijks',
        dueToday: '€0 vandaag betalen',
        chargeNote: 'Eerste betaling {when}.',
        chargeWhen: 'wanneer je formule in productie gaat',
      },
    },
    legal: {
      privacy: 'Privacy',
      terms: 'Algemene voorwaarden',
      imprint: 'Colofon',
      gdpr: 'AVG',
      copyright: '© NB1 Health GmbH 2026',
    },
  },
  journal: {
    heroTitle: 'De darm, uitgelegd.',
    heroLede:
      'Onderbouwde artikelen over het microbioom, voeding en de wetenschap achter een formule die alleen voor jou wordt gemaakt. Geschreven en gecontroleerd door het NB1-team.',
    allTopics: 'Alles',
    readArticle: 'Lees het artikel',
    minRead: 'min leestijd',
    featuredLabel: 'Uitgelicht',
    empty: 'Nog geen artikelen over dit onderwerp.',
    imagePlaceholder: 'Afbeelding',
    references: 'Bronnen',
    keepReading: 'Verder lezen',
    by: 'Door',
    reviewedBy: 'Gecontroleerd door',
    breadcrumbHome: 'Home',
    breadcrumbJournal: 'Journal',
    navOverview: 'Overzicht',
    breadcrumbPage: 'Pagina',
    complianceLabel: 'Opmerking over claims:',
    ctaHeading: 'Gebouwd op jouw biologie, niet op een aanname.',
    ctaBody: 'NB1 leest jouw darm en bouwt een formule die alleen voor jou is gemaakt.',
    ctaLabel: 'Bekijk hoe het werkt',
    ctaPath: '/your-plan',
    exploreHubs: 'Ontdek per onderwerp',
    relatedTopics: 'Gerelateerde onderwerpen',
    lastReviewed: 'Laatst gecontroleerd',
    aboutAuthor: 'Over de auteur',
    profile: 'Bekijk het volledige profiel',
    relatedResearch: 'Gerelateerd onderzoek',
    relatedReading: 'Verder lezen',
  },
}
