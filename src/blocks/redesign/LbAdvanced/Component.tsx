...
import type { RdLbAdvancedBlock as Props } from '@/payload-types'

// GENERATED from manifests/section-08.json + bindings/RdLbAdvanced.json by
...
// `rd-lb rd-lbadvanced` and NOT `rd-block`.
//
// Three cards bound by index — each has its own icon and chip colour — with bullet lists inside them that genuinely repeat.


export const RdLbAdvanced: React.FC<Props> = ({ anchorId, cards, closing, heading, intro, tag }) => {
  return (
    <section style={{
      background: "var(--nb1-dark-brown)",
      color: "var(--nb1-cool-grey)"
    }} className="rd-lb rd-lbadvanced" id={anchorId || undefined} data-m="dark">
      <div style={{
        maxWidth: "1240px",
        margin: "0px auto",
        padding: "64px 20px"
        ...
        <div style={{
          ...
          {(closing)?.root ? (
            <RichText data={closing} enableGutter={false} enableProse={false} />
          ) : null}
        </div>
      </div>
    </section>
...
export const RdLbAdvancedComponent = RdLbAdvanced

export default RdLbAdvanced
