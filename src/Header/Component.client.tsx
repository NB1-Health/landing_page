...
export const HeaderClient: React.FC<HeaderClientProps> = ({
  ...
  // The language / currency selector lives in a shared hook now — see
  // src/components/LocaleCurrency/useLocaleCurrency.ts. It was moved out
  // VERBATIM so the redesign header can drive the real switcher instead of
  // drawing a decorative copy of it. The JSX below is untouched: the hook is
  // headless, so what this header renders is byte-for-byte what it rendered
  // before.
  const {
    activeLang,
    curLang,
    ...
    curSym,
    applyLang,
    // resolved by the hook, so its defaults apply and these are never undefined
    langs,
  } = useLocaleCurrency({
    locale,
    initialCurrency,
